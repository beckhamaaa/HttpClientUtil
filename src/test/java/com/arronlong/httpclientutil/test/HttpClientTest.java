...
//        testOne();
    ...
    public static void testOne() throws HttpProcessException {
        ...
        log.info("请求结果内容长度：" + resp.length() + " headers:" + Arrays.toString(headers));

        System.out.println("--------代理设置（绕过证书验证）-------");
        url = "https://www.facebook.com/";
        //采用默认方式（绕过证书验证）
//		HttpClient client= HCB.custom().timeout(10000).proxy("127.0.0.1", 8087).ssl().build();
//		//执行请求
...
//				"http://blog.csdn.net/xiaoxian8023/article/details/43312929", 
        ...
        try {
            ...
            for (int i = 0; i < pagecount * 10; i++) {
                FileOutputStream out = new FileOutputStream(new File("d://" + (i + 1) + ".png"));
                //启动线程抓取
