...
public class TestCookie {
    ...
    public static void main(String[] args) throws HttpProcessException {
        ...
        //获取参数, 可以用.send(config)代替，但是推荐使用明确的get方法
        String loginform = HttpClientUtil.get(config);
        log.info("获取登录所需参数" + loginform);
        ...
        //发送登录请求, 可以用.send(config.method(HttpMethods.POST).map(map))代替，但是推荐使用明确的post方法
        String result = HttpClientUtil.post(config.map(map));
        //System.out.println(result);
        if (result.contains("帐号登录")) {
            //如果有帐号登录，则说明未登录成功
...
//		}
        ...
        //可以用.send(config.url(scoreUrl).headers(headers))代替，但是推荐使用明确的post方法
        result = HttpClientUtil.post(config.url(scoreUrl).headers(headers));
        //获取C币
